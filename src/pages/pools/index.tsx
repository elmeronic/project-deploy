...
const Pools = () => {
  ...
  const get_token_pool_pair_info = (pages: any) => {
    let token_pair_info = [];
  ...
  const toDetail = (tokenA_id: any, tokenB_id: any) => {
    router.push(`/pools/detail/${tokenA_id}/${tokenB_id}`);
  };

  const firstPage = async () => {
    const token_pool_pair_info: any = get_token_pool_pair_info(1);
    setLocalTokenPoolPair(token_pool_pair_info);
    setPages(1);
  };

  const lastsPage = async () => {
    const token_pool_pair_info: any = get_token_pool_pair_info(pagesLast);
    setLocalTokenPoolPair(token_pool_pair_info);
    setPages(pagesLast);
  };

  const leftPage = async (pages: any) => {
    if (pages >= 1) {
      const token_pool_pair_info: any = get_token_pool_pair_info(pages);
      setLocalTokenPoolPair(token_pool_pair_info);
      setPages(pages);
    }
  };
  const rightPage = async (pages: any) => {
    if (pages <= pagesLast) {
      const token_pool_pair_info: any = get_token_pool_pair_info(pages);
      setLocalTokenPoolPair(token_pool_pair_info);
      setPages(pages);
              ...
              <div className="w-full mt-2   overflow-auto">
                <table className="min-w-full divide-y divide-W3GInfoBorderBG">
                  ...
                  <tbody className="  divide-y divide-W3GInfoBorderBG">
                    {token_watchlist_pool_pair.map((item: any) => (
                      <tr
                        key={item.pool_id}
                        onClick={() => {
