              ...
              <div className="inline-block align-bottom p-0.5 rounded-lg bg-gradient-to-br from-W3G1  via-W3G2 to-W3G3 w-11/12 md:w-5/12 2xl:w-4/12  rounded-lg  text-left overflow-hidden shadow-xl transform transition-all sm:y-8 sm:align-middle   ">
                <div className="bg-black px-4 py-5 sm:px-6 lg:px-12 rounded-md">
                  <div className="flex justify-between text-xl font-light text-white 	mb-5">
                    <div className=" font-light text-base text-white font-semibold ">Select a token</div>
                    <button onClick={() => setSelectToken(false)} className="fa fa-times " aria-hidden="true"></button>
                  </div>
                  <div className="flex ">
                    ...
                    <button className="ml-4 p-0.5 w-32 rounded-lg text-white bg-gradient-to-r from-[#DA6081] via-[#8D6BCD]  to-[#7092E7]">
                      Add Token
                    </button>
                  </div>
                  <div className="my-5 text-white font-semibold">Common bases</div>
                  ...
                  <div className="overflow-y-auto border-t border-gray-700 h-64 pr-3  scrollbar-thin scrollbar-thumb-custom  scrollbar-thumb-rounded-full  overflow-y-scroll ">
                    ...
                    {tokenList.map(item => (
                      <div
                        key={item.name}
                        onClick={() => select(item)}
                        className="cursor-pointer flex justify-between mr-2 items-center"
                      >
                        <div className="flex my-2">
                        ...
                        <div className="mt-3 text-gray-300">{item.data}</div>
                      </div>
                    ))}
                  </div>

                  <div className="mt-2 flex justify-center text-gray-300 font-semibold ">
                    <button onClick={openTokenLists}>Manage Token Lists</button>
                  </div>
                </div>
              </div>
            </Transition.Child>
