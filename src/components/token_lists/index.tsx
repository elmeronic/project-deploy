...
const List = () => {
  ...
  const setEnabled = (index: any) => {
    let data: any = tokenList.concat();
            ...
            <div className="flex ">
              ...
              <div className="text-white ml-4 ">
                <div className="flex items-center">
                  {item.token}
                  <div className="text-sm ml-2">{item.version}</div>
                </div>
                <div className="text-xs text-gray-300">{item.tokenNumber} tokens</div>
              </div>
            </div>
            <Switch
...
const Tokens = () => {
  ...
  const [customTokenList, setCustomTokenList]: any = useAtom(custom_token_list);
  ...
  const DeleteTokensPopUpBox = (e: any) => {
    setRemoveTokenDateName(e);
    setRemoveTokenList(true);
  };
  const deleteTokens = (e: any) => {
  ...
  return (
    <>
      ...
      <button
        ...
        className={valuable ? 'flex mt-5 w-full border border-gray-700 bg-W3GInfoBG p-2 px-3 rounded-xl' : 'hidden'}
      >
        <div className="flex  items-center  ">
          ...
          <div className="text-white ml-4 ">
            <div className="flex items-center">
              ...
              <div className="ml-2 text-xs py-1 px-2 bg-[#6BA2E5] rounded-full">Unknown Source</div>
            </div>
      ...
      <div className="my-5 rounded-xl    bg-W3GInfoBG  border-gray-700 border p-3 py-2 ">
        <div className="flex justify-between items-center">
          <div className="flex">
            ...
            <div className="ml-1">Custom Tokens</div>
          </div>
        ...
        <div
          className={
            customTokenList.length
              ? 'h-60 overflow-y-auto mt-2  scrollbar-thin scrollbar-thumb-custom  scrollbar-thumb-rounded-full  overflow-y-scroll'
              : 'hidden'
          }
        >
          {customTokenList.map((item: any) => (
            <div key={item.token} className="flex items-center justify-between mt-2">
              <div className="flex items-center">
                <img className="w-10" src={item.img} alt="" />
                <div className="ml-3">{item.token}</div>
              </div>
              <div className="flex text-xl mr-2 items-center">
                <button
                  onClick={() => {
                    DeleteTokensPopUpBox(item.token);
                  }}
                  className="mr-5"
                >
                ...
                <div className="bg-black px-4 py-5 sm:px-6 lg:px-12 rounded-md">
                  <div className="flex justify-between text-xl font-light text-white 	mb-5">
                    <div className=" flex items-center">
                      ...
                      <div className="font-semibold ml-4 text-xl">Import token</div>
                    </div>
                    <button
                      onClick={() => {
                        setImportToken(false);
                  ...
                  <div className="flex justify-center py-10">
                    <button
                      onClick={add_token_in_list}
                      className="px-5 py-2 w-56 text-white mt-6 rounded-lg  bg-gradient-to-r from-W3G1 via-W3G2 to-W3G3"
                    >
                      Import
                    </button>
                  </div>
                ...
                <div className="bg-black px-4 py-5 sm:px-6 lg:px-12 rounded-md">
                  <div className="w-full  py-10">
                    <div className="flex justify-center text-xl text-white w-full">
                      <div>Are you sure you want to delete all listings ？</div>
                    </div>
                  </div>
                  ...
                  <div className="text-center mt-5 flex justify-between">
                    <div className="mt-1">
                      <button
                        ...
                        className=" lg:mt-0  w-36 px-3 py-2 rounded-lg bg-gradient-to-r from-W3G1 via-W3G2 to-W3G3 text-white"
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <div className="w-full  py-10">
                    <div className="flex justify-center md:text-xl text-white w-full">
                      <div className="flex">
                        ...
                        <div className="px-0.5 underline">{removeTokenDateName}</div>？
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-center mt-5 flex justify-center">
                    <div className="mt-1">
                      <button
                        ...
                        className=" lg:mt-0  mr-10  w-36 px-3 py-2 rounded-lg bg-gradient-to-r from-W3G1 via-W3G2 to-W3G3 text-white"
                      >
                        Cancel
                      </button>
                    </div>
                    <div className="mt-1">
                      <button
                        onClick={() => deleteTokens(removeTokenDateName)}
                        className=" lg:mt-0  w-36 px-3 py-2 rounded-lg bg-gradient-to-r from-W3G1 via-W3G2 to-W3G3 text-white"
                      >
                        Confirm
                      </button>
                ...
                <div className="bg-black px-4 py-5 sm:px-6 lg:px-12 rounded-md">
                  <div className="flex justify-between text-xl font-light text-white items-center mb-5">
                    <div className=" flex items-center">
                      ...
                      <div className="font-light ml-4 text-xl text-white">Select a token</div>
                    </div>
                    <button
                      onClick={() => {
                        setCloseTokenList(false);
                  ...
                  <Tab.Group>
                    ...
                    <Tab.Panels className="mt-2 ">
                      <Tab.Panel className={classNames(' rounded-xl p-1')}>
                        <List />
                      </Tab.Panel>
                      {/*Popular*/}
